/**
 * Static file server for production builds (Chat App)
 *
 * Serves the built chat frontend files in production mode.
 */
...
const logger = createLogger('ChatStaticServer');
...
export async function startStaticServer(): Promise<void> {
  const staticPath = path.join(electronDir, '../dist');
